    ...
    <div v-else class="grid gap-3 grid-cols-1 sm:grid-cols-2">
      <Card v-for="s in store.secrets" :key="s.id" color="amber">
        <div class="flex items-start justify-between gap-3 mb-2">
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              ...
              <p class="text-[10px] text-arena-500 font-mono">{{ s.key }}</p>
            </div>
          </div>
        ...
        <div class="flex items-center gap-2">
          <Badge variant="muted">${{ s.key }}</Badge>
          <p v-if="s.description" class="text-[10px] text-arena-500 truncate">{{ s.description }}</p>
        </div>
      </Card>
    </div>
