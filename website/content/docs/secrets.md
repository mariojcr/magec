...
In the Admin UI, go to **Secrets** and click **New**:

| Field | Description |
|-------|-------------|
| `name` | Display name — a human-readable label (e.g., "OpenAI Production Key") |
| `key` | Environment variable name — must be `UPPER_SNAKE_CASE` (e.g., `OPENAI_API_KEY`). This is what you reference with `${KEY}` in your configuration. |
| `value` | The sensitive value itself. Write-only — once saved, it's never shown again in the UI or API. |
| `description` | Optional note about what this secret is for or where it's used. |

Keys must be unique. If you try to create a secret with a key that already exists, the request is rejected.

### Write-only values

Secret values are **never returned** by the API or displayed in the UI after creation. When you edit a secret, the value field appears empty. If you submit the edit without entering a new value, the existing value is preserved — you only need to fill it in when you want to change it.
...
Reference secrets anywhere `${VAR}` expansion is supported:
...
```json
{
  "name": "Redis Sessions",
  "type": "redis",
  "connectionString": "redis://${REDIS_PASSWORD}@redis:6379"
}
```
...
The `${VAR:-default}` syntax works too — if the variable is unset or empty, the default value is used.
